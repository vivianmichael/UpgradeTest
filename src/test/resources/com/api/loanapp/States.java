...
public enum States {
    ...
    "Delaware", "DE"), DISTRICT_OF_COLUMBIA("District of Columbia", "DC"), FEDERATED_StatesS_OF_MICRONESIA(
    "Federated Statess of Micronesia", "FM"), FLORIDA("Florida", "FL"), GEORGIA("Georgia", "GA"), GUAM("Guam", "GU"), HAWAII(
    ...
    public static Map<String, States> StatesS_BY_ABBR = new HashMap<String, States>();
    
    /* static initializer */
    static {
        for (States States : values()) {
            StatesS_BY_ABBR.put(States.getAbbreviation(), States);
        }
    }

    /**
     * Constructs a new States.
     *
     * @param name the States's name.
     * @param abbreviation the States's abbreviation.
     */
    ...
    /**
     * Returns the States's abbreviation.
     *
     * @return the States's abbreviation.
     */
    ...
    public static States valueOfAbbreviation(final String abbr) {
        final States States = StatesS_BY_ABBR.get(abbr);
        if (States != null) {
            return States;
        } else {
            return UNKNOWN;
